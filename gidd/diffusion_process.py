...
class NoiseSchedule(nn.Module, ABC):
    def __init__(self, tokenizer):
        ...
        self.vocab_size = tokenizer.unk_token_id # TODO: Assumption: tokens which can be generated by the diffusion process appear first in the vocab, the unknown token is the first token after
...
class HybridDiffusion(NoiseSchedule):
    ...
    def probs_at_t(self, prs, t, eps=1e-4):
        ...
        B = self.log_B.exp()
        c_t = t_gamma.sqrt() * t1m_gamma.sqrt() * B
        C_t = t_gamma + t1m_gamma + (self.vocab_size) * c_t # TODO: why t_gamma + t1m_gamma instead of just 1?
