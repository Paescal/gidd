...
class Sampler(nn.Module):
    ...
    @abstractmethod
    def _do_generate_from_given(self, initial_z_t, diffusion_mask, solution, num_denoising_steps, num_self_correction_steps, max_length, show_progress, device, keep_history=False):
        raise NotImplementedError
    ...
    @torch.no_grad()
    def generate_from_given(self, z_t, diffusion_mask, solution=None, puzzle_conditioning=None, num_denoising_steps=128, num_self_correction_steps=0, max_length=None, decode=True, show_progress=True, keep_history=False):
        ...
        if puzzle_conditioning is None:
            z_t, history = self._do_generate_from_given(z_t, diffusion_mask=diffusion_mask, solution=solution, puzzle_conditioning=z_t.clone(), num_denoising_steps=num_denoising_steps, num_self_correction_steps=num_self_correction_steps, max_length=max_length, show_progress=show_progress, device=device, keep_history=keep_history)
        else:
            z_t, history = self._do_generate_from_given(z_t, diffusion_mask=diffusion_mask, solution=solution, puzzle_conditioning=puzzle_conditioning, num_denoising_steps=num_denoising_steps, num_self_correction_steps=num_self_correction_steps, max_length=max_length, show_progress=show_progress, device=device, keep_history=keep_history)
...
class GiddSampler(Sampler):
    ...
    def _do_generate_from_given(self, initial_z_t, diffusion_mask, solution, puzzle_conditioning, num_denoising_steps, num_self_correction_steps, max_length, show_progress, device, keep_history=False):
        ts = torch.linspace(0, 1, num_denoising_steps + 1, device=device).unsqueeze(-1)
        ts = (1 - 2 * self.t_eps) * ts + self.t_eps
