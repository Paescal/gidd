        ...
        for dataset in "${datasets[@]}"; do
            for num_denoising_steps in "${nums_denoising_steps[@]}"; do
                for num_self_correction_steps in "${nums_self_correction_steps[@]}"; do
                    suffix="dataset=${dataset} num_samples=$num_samples num_denoising_steps=$num_denoising_steps num_self_correction_steps=$num_self_correction_steps batch_size=$batch_size min_p=$min_p compile_torch=$compile_torch seed=$seed"
...
echo "accuracy,correctly_filled_cells,not_fully_unmasked,checkpoint,strategy,params,denoised_fraction,mask_fraction,uniform_fraction,history" > $csv_file
...
cat $combinations_file | parallel --colsep ',' -j $num_jobs '
    ...
    echo "$ACCURACY,$CORRECTLY_FILLED_CELLS,$NOT_FULLY_UNMASKED,$CKPT,$STRATEGY,\"$PARAMS_ORIGINAL\",$DENOISED_FRACTION,$MASK_FRACTION,$UNIFORM_FRACTION,$HISTORY" >> $csv_file
